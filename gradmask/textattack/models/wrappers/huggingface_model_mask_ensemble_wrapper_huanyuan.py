...
from transformers import PreTrainedTokenizer, AutoModelForMaskedLM, RobertaTokenizer , RobertaForMaskedLM
...
class HuggingFaceModelMaskEnsembleWrapper(PyTorchModelWrapper):
    ...
    def __init__(self, 
        ...
        if args.with_lm:
            ...
            self.masked_lm.eval()
        ROBERTa_PATH = '/root/xuce/RanMaskT1/RanMASK/save_models/roberta-base'
        self.masked_ml_tokenizer=RobertaTokenizer.from_pretrained(ROBERTa_PATH)
        self.masked_model = RobertaForMaskedLM.from_pretrained(ROBERTa_PATH)
    ...
    def masked_ml(self, masklist):
        #ROBERTa_PATH = '/root/xuce/RanMaskT1/RanMASK/save_models/roberta-base'
        huanyuan_list = []
        #tokenizer = RobertaTokenizer.from_pretrained(ROBERTa_PATH)
        #mask_model = RobertaForMaskedLM.from_pretrained(ROBERTa_PATH)
        #mask_model.eval()
        for i in masklist:
            #tokenized_text = tokenizer.tokenize(i)
            tokenized_text =self.masked_ml_tokenizer.tokenize(i)
            #indexed_tokens = tokenizer.convert_tokens_to_ids(tokenized_text)
            #indexed_tokens = tokenizer(i)['input_ids']
            indexed_tokens = self.masked_ml_tokenizer(i)['input_ids']
            ...
            for x in range(len(tokenized_text)):
                if tokenized_text[x] == '<mask>':
                    alist.append(x)
            #masked_index = tokenized_text.index('<mask>')
            # Predict all tokens
            with torch.no_grad():
                #predictions = mask_model(tokens_tensor, segments_tensors)
                predictions = self.masked_model(tokens_tensor, segments_tensors)
            for j in alist:
                #predicted_index = torch.argmax(predictions[0][0][j+1]).item()
                predicted_index_k = torch.topk(predictions[0][0][j+1],10,dim=-1,largest=True, sorted=True, out=None)
                predicted_index = predicted_index_k[1]
                #0表示top-1的单词
                predicted_token = self.masked_ml_tokenizer.convert_ids_to_tokens([predicted_index[0]])[0]
                predicted_token = predicted_token.replace('Ġ', '')
                i=i.replace('<mask>', predicted_token, 1)
            huanyuan_list.append(i)
        return huanyuan_list
    ...
    def __call__(self, text_input_list):
        ...
        if isinstance(text_input_list[0], tuple) and len(text_input_list[0]) == 2:
            ...
            for text_input in text_input_list:
                temp1 = self.mask_sentence_decrator(text_input) 
                temp2 = self.masked_ml(temp1)
                mask_text_input_list.extend(temp2)      
