...
class HuggingFaceModelMaskEnsembleWrapper(PyTorchModelWrapper):
    ...
    def mask_sentence_decrator(self, sentence:str,unattack:str) -> List[str]:
        ...
        return mask_sentence(sentence, unattack, self.mask_rate, self.mask_token, self.ensemble, forbidden=forbidden_index, random_probs=random_probs )
    ...
    def __call__(self, text_input_list):
        ...
        """
        add_nums=len(text_input_list)-1
        unattack=text_input_list[add_nums]
        text_input_list.pop()
        mask_text_input_list = []
        if isinstance(text_input_list[0], tuple) and len(text_input_list[0]) == 2:
            for text_input in text_input_list:
                mask_text_input = [(text_input[0], sentence) for sentence in self.mask_sentence_decrator(text_input[1],unattack[1])]
                mask_text_input_list.extend(mask_text_input)
        else:
            for text_input in text_input_list:
                mask_text_input_list.extend(self.mask_sentence_decrator(text_input,unattack))
