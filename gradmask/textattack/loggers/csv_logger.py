...
class CSVLogger(Logger):
    ...
    def flush(self):
        if os.path.exists(self.filename):
            self.df.to_csv(self.filename, mode='a', header=False, quoting=csv.QUOTE_NONNUMERIC, index=False)
        else:
            self.df.to_csv(self.filename, mode='w', header=True, quoting=csv.QUOTE_NONNUMERIC, index=False)
        self._flushed = True
