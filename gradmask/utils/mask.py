...
def mask_sentence(sentence: str, 
                  ...
                  random_probs: List[int] = None, 
                  min_keep: int = 2) -> List[str]:
    sentence_in_list = sentence.split()

    tmp_sentences = []
    # Z=len(random_probs)
    # random_probs=np.random.choice(len(sentence_in_list),Z,replace=False,p=None)
    tmp_sentence = mask_sentence_by_indexes(sentence_in_list, random_probs, token, forbidden)
    tmp_sentences.append(tmp_sentence)
    # tmp_sentences=[]
    # for indexes in random_probs:
    #     tmp_sentence = mask_sentence_by_indexes_single(sentence_in_list, indexes, token)
    #     tmp_sentences.append(tmp_sentence)       
    if return_indexes:
        return tmp_sentences, random_probs
    else:
...
def mask_sentence_by_indexes_single(sentence: List[str], indexes, token: str) -> str:
    tmp_sentence = sentence.copy()
    tmp_sentence[indexes] = token
    return ' '.join(tmp_sentence)