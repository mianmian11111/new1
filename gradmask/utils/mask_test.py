...
def sampling_index_loop_nums(length: int,
    ...
    mask_indexes = []
    for _ in range(nums):
        temp = RanMask(length, rate)
        xiangling = panduan(temp)
        while(xiangling == 1):
            temp = RanMask(length, rate)
            xiangling = panduan(temp)
        mask_indexes.append(temp)
    return mask_indexes


def RanMask(length, rate):
    temp_mask_list = []
    for i in range(length):
        random.seed()
        c = random.random()
        if c <= rate:
            temp_mask_list.append(i)
    return temp_mask_list


def panduan(mask_index_temp):
    if len(mask_index_temp) == 0: 
        return 0
    for i in mask_index_temp:
        for j in mask_index_temp[1:]:
            if (abs(i-j) == 1):
                return 1 
    return 0
...
def mask_sentence(sentence: str, 
    ...
    mask_indexes = sampling_index_loop_nums(length, rate, nums, random_probs)
    tmp_sentences = []
...
def mask_sentence_by_indexes(sentence: List[str], indexes: np.ndarray, token: str, forbidden: List[str]=None) -> str:
    ...
    index_map = [x for x in indexes]
    for index in index_map:
        temp1 = tmp_sentence[0:index]
        temp2 = tmp_sentence[index+2:len(tmp_sentence)]
        temp1.append(token)
        tmp_sentence = temp1+temp2
        for i,v in enumerate(index_map):
            index_map[i] = v-1
