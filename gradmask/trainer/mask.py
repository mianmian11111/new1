    ...
    def mask_batch_instances(self, instances: List[InputInstance], rate: float, token: str, nums: int = 1) -> List[InputInstance]: 
        ...
        for instance in instances:
            ...
            if self.masked_lm is not None:
                random_probs =self.mask_probs(instance.text_a)
            batch_instances.extend(mask_instance(instance, rate, token, nums, forbidden_indexes=forbidden_index,random_probs=random_probs))
        return batch_instances
    
    def mask_probs(self, sentence: str) -> List[int]:
        b=self.cipin(self.dictt,sentence)
        return b
